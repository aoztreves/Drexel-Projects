		...
		if(selection == 1){ //Survey
			...
			while(selection != 8){
				...
				case 2:
					...
					if((voicer.equals("yes"))||(voicer.equals("Yes"))){
						voicer = "yes";
					}
					else{
						voicer = "no";
					...
					for(int i=0; i < survey1.QA.size(); i++){ //Display
						...
						//voice.speak(survey1.QA.get(i).prompt);
						//voice.speak("Choices");
						//voice.speak(survey1.QA.);
						
						survey1.QA.get(i).display(voicer);
					}
					break;
				case 3: // Load
			...
			while(selection != 9){
				...
				case 2://Display
					...
					for(int i=0; i < test1.QA.size(); i++){
						System.out.print(i+1 + ") ");
						test1.QA.get(i).display(voicer); //Display question
						
						//System.out.println("hey hey");
						
						Vector<String> choices =test1.getCorrectAnswer().get(i).getuserAnswers();
						if(choices.get(0) != ""){
							if(voicer.equals("yes")){
								

								voice.allocate();
								voice.speak("Correct Answer is");
								voice.deallocate();
							}
							else{
